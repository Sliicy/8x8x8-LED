 ...
 * Example:
 * 
 * To update the entire cube to Red, assuming the colorDepth is set to 1, the following 4 payloads need to be sent (in any order):
 * 
 * 0x00,0xFF,0x00,0x00, // Header
 * 0x00,0x00, // Color Depth, Z-Axis
 * 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x00, 0x00, // Payload
 ...
 *  2 bytes prepended corresponding to depth and height to render):
 */
const byte numBytes = yAxisStates * 2 + 2;
...
int targetColorDepth = 0; // Contains the destination cube that is to be rendered.
int targetZLayer = 0; // Contains the destination of the 2 Z-Axes to be rendered.
...
void setup() {
  ...
  // Open serial port and listen @ 2000000 bps:
...
void loop() {
  ...
  if (newData) {
    ...
    targetZLayer = receivedBytes[1];
    for (int y = 0; y < yAxisStates; y++) {
      
      // Render the target cube and 2 target Z-Axis layers, skipping 2 bytes which specify what to render:
      cube[targetColorDepth][targetZLayer][y] = receivedBytes[y + 2];
      cube[targetColorDepth][targetZLayer + 1][y] = receivedBytes[yAxisStates + y + 2];
    }
    newData = false;
  }
